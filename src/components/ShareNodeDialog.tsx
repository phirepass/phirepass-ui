...
export function ShareNodeDialog({ open, onOpenChange, node, onManageShares }: ShareNodeDialogProps) {
  ...
  return (
    ...
    <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
            Share access to <span className="font-medium text-foreground">{node.name}</span> with another user via a one-time link.
        </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-lg border border-border bg-secondary/50 p-3 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">{node.ip} • {node.os}</p>
